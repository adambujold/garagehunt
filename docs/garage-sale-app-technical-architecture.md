...
### Matching & notifications (Phase 2d)
- On listing publish (the real stored transition is `status` becoming `published` — "scheduled"/"live"/"ended" are display-only states derived from `start_date`/`end_date`, never written to the database): a database trigger on `sale_listings_raw` (`supabase/migrations/0035_server_side_match_computation.sql`) scans `saved_searches` within the listing's radius, inserts `matches` rows for category/keyword hits. This runs identically regardless of which client (mobile app or companion website) performed the publish — neither client calls anything to trigger it, unlike the original client-side `computeAndInsertMatches` this replaced.
- Category matching is exact-set overlap. Keyword matching against free-text `description`/`other_items` is case-insensitive substring matching (not full-text/embedding-based) — same as Discover's own "Other" keyword filter, so match behavior stays consistent with what a buyer sees when searching manually.
- A `matches` row insert (from either the trigger above or the one remaining client-side path — backfilling matches for a newly-saved search against already-published listings) fires a webhook trigger that calls the `send-match-notification` Edge Function, which sends both a push notification (Expo) and an immediate email (Resend) per match — not a batched digest. Email currently only reliably delivers to Resend's own verified sender address until a verified sending domain is configured; the code path is unaffected either way.
