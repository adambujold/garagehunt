...
- **⚠️ Security fix: `moderation_status` must be server-side-only, never client-writable.** A real gap existed on both `sale_listings.moderation_status` and `listing_photos.moderation_status`: the client set them directly, so a dishonest seller could mark their own listing `clean` (or a photo `approved`) and skip review entirely, whatever the real check found. Client write access to both is now revoked; only service-role Edge Functions write them.

  **The decision must be computed and written in the same server-side operation — never returned to the client to apply.** This is the part that actually closes the hole, and it's why the fix isn't simply "revoke the column": a verdict handed back to a client is a verdict the client can ignore. Two consequences worth stating explicitly, because both were real holes:
  - **`publish-listing`** (not `moderate-listing-text`) owns the listing decision. It reads the description **from the stored row**, classifies it, applies the new-account rule, and writes `status` + `moderation_status` itself. `moderate-listing-text` only ever receives loose text and returns a verdict — it has no listing id and cannot safely write anything. Accepting the text from the request would recreate the hole one level down: submit innocuous copy for screening while the row holds something else.
  - **`moderate-listing-photo`** owns the photo decision end to end — it receives the image, classifies it, uploads the approved bytes to Storage itself, and inserts the `listing_photos` row with the verdict it computed. Clients no longer have INSERT on `listing_photos` or on the bucket, so the row can only describe an image the function actually screened.

  See `0041_server_side_publish_gate.sql` and `0042_server_side_photo_moderation.sql` for the enforcement.
...
- ~~Time-sensitive: legacy Supabase JWT-format API keys~~ — **Resolved, correction to earlier note.** This doc previously warned that Supabase's newer `sb_secret_...` key format would fail Edge Functions' JWT verification outright, based on direct testing at the time. That premise is **no longer true** — Supabase has since updated their gateway. Confirmed two ways: the webhook Edge Functions are now running on the new-format key and real notifications (day-of-photo push and email) are delivering successfully through it; separately, a direct test of a non-JWT `sb_` key returned a real 200 through the gateway. No migration to a custom shared-secret header is needed — the original concern was accurate when written, but Supabase's own infrastructure changed underneath it. Nothing to act on here.
